...
int isFull() {                            // Check if queue is full
    return (f == 0 && r == MAX-1) || (f == r+1);  // Front at 0 & rear at end, or full circle
}

int isEmpty() {                           // Check if queue is empty
    return f == -1;                       // -1 indicates no elements
}

void insert(int data) {                   // Add element at rear
    if( isFull() ) {                      // Check space availability
        printf("Queue Full!\n");
        return;
    }
...
void delete() {                           // Remove element from front
    if( isEmpty() ) {                     // Check if queue has elements
        printf("Queue Empty!\n");
        return;
    }
    printf("Deleted: %d\n", q[f]);        // Show deleted element
    if(f == r)                            // Last element case
        f = r = -1;
    else
        f = (f + 1) % MAX;                // Move front circularly
}
